...
```{code-cell} ipython3
df = pd.read_csv('data/marathon.txt', names = names, header=0, sep = '\t')
df
```
...
```{code-cell} ipython3
df.to_csv('data/marathon.txt', sep='\t')
```
...
# Si on garde la colonne duration comme telle, on obtient une erreur de format.
# il faut donc faire :
df['duration'] = pd.to_timedelta(df['duration'], unit='s', errors='coerce')
#durmoy = df.pivot_table(
#    values='duration',
#    index='city',
#    aggfunc='mean')

#plus simple qu'un TCD : 
durmoy = df.groupby('city')['duration'].mean()
print(durmoy)

#il faudrait pouvoir nettoyer les données en sortie.
```

affichez la moyenne des durées des marathons par ville triée en ordre croissant

```{code-cell} ipython3
durmoycrois = durmoy.sort_values()
print(durmoycrois)
```
...
df['second'] = ((df['seconds'] % 3600) % 60) // 1
```
