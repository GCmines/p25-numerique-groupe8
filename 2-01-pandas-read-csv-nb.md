...
```{code-cell} ipython3
d = pd.read_csv('data/petit-titanic.csv')
d.head(2)
#On se doute au vu du résultat que le problème vient du séparateur.
```
...
```{code-cell} ipython3
NomColonnes = ['PassengerId', 'Survived', 'Class', 'Name', 'Sex', 'Age', 'SibSp', 'Parch', 'Ticket', 'Fare', 'Cabin', 'Embarked']
d = pd.read_csv('data/petit-titanic.csv',sep=';',header=None,names=NomColonnes)
```
...
```{code-cell} ipython3
print("index colonnes : ", d.columns, " et index lignes : ", d.index)
```
