...
```{code-cell} ipython3
(events['beg'] < events['end']).all()
```

```{code-cell} ipython3
#events = events.sort_values(by='beg')
events = events.set_index('beg').sort_index()
events.head(10)
```
...
```{code-cell} ipython3
events['end+1'] = events['end'].shift(1)
(events.index < events['end+1']).all()

#Un "false" en sortie signifierait qu'il n'y a pas d'overlap dans nos évènements.
```
...
```{code-cell} ipython3
events['durée'] = events['end'] - events.index
#events.head(10)
temtot = events['durée'].sum()
ratio = temtot / timespan
print('temps total de réservation: ', temtot, '\n', 'temps total : ', timespan, '\n', 'ratio : ', ratio)
```
...
sem = events['durée'].resample('W').sum()
mois = events['durée'].resample('ME').sum()
ann = events['durée'].resample('YE').sum()
...
```{code-cell} ipython3
#Aidé par l'IA pour cette question comme conseillé dans l'indice


fig, ax = plt.subplots(figsize=(14,4))
sem.plot(kind='bar', ax=ax)
ax.set_title("Durée totale des événements par semaine")
ax.set_ylabel("Durée (jours)")
ax.set_xticks(range(0, len(sem), SPACES['W']))
ax.set_xticklabels([str(x.date()) for x in sem.index[::SPACES['W']]], rotation=45, fontsize=8)
plt.show()

fig, ax = plt.subplots(figsize=(14,4))
mois.plot(kind='bar', ax=ax)
ax.set_title("Durée totale des événements par mois")
ax.set_ylabel("Durée (jours)")
step = 3
ax.set_xticks(range(0, len(mois), SPACES['M']))
ax.set_xticklabels([x.strftime('%Y-%m') for x in mois.index[::SPACES['M']]], rotation=45, fontsize=8)
plt.show()

fig, ax = plt.subplots(figsize=(14,4))
ann.plot(kind='bar', ax=ax)
ax.set_title("Durée totale des événements par année")
ax.set_ylabel("Durée (jours)")
step = 1
ax.set_xticks(range(0, len(ann), SPACES['Y']))
ax.set_xticklabels([x.strftime('%Y') for x in ann.index[::SPACES['Y']]], rotation=45, fontsize=8)
plt.show()
```
...
def convert_timedelta_to_hours(timedelta: pd.Timedelta) -> int:
    heures = timedelta.total_seconds() // 3600
    #heures = timedelta.total_seconds() / 3600
    #return heures
...
```{code-cell} ipython3
sem_hours = sem.map(convert_timedelta_to_hours)
mois_hours = mois.map(convert_timedelta_to_hours)
ann_hours = ann.map(convert_timedelta_to_hours)


#On reprend le code de tout à l'heure et on l'adapte

ax = sem_hours.plot(kind='bar', figsize=(14,4))
ax.set_title("Durée totale des événements par semaine (en heures)")
ax.set_ylabel("Durée (heures)")
ax.set_xticks(range(0, len(sem_hours), SPACES['W']))
ax.set_xticklabels([str(x.date()) for x in sem_hours.index[::SPACES['W']]], rotation=45, fontsize=8)
plt.tight_layout()
plt.show()

ax = mois_hours.plot(kind='bar', figsize=(14,4))
ax.set_title("Durée totale des événements par mois (en heures)")
ax.set_ylabel("Durée (heures)")
ax.set_xticks(range(0, len(mois_hours), SPACES['M']))
ax.set_xticklabels([x.strftime('%Y-%m') for x in mois_hours.index[::SPACES['M']]], rotation=45, fontsize=8)
plt.tight_layout()
plt.show()

ax = ann_hours.plot(kind='bar', figsize=(14,4))
ax.set_title("Durée totale des événements par année (en heures)")
ax.set_ylabel("Durée (heures)")
ax.set_xticks(range(0, len(ann_hours), SPACES['Y']))
ax.set_xticklabels([x.strftime('%Y') for x in ann_hours.index[::SPACES['Y']]], rotation=45, fontsize=8)
plt.tight_layout()
plt.show()
```
