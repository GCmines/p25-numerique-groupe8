...
```{code-cell} ipython3
df = pd.DataFrame(by_country['France'])
print(df)
```
...
```{code-cell} ipython3
print('Les colonnes sont : ', df.columns, ' et il y a ', len(df), ' mesures.')
```
...
```{code-cell} ipython3
dfi = pd.DataFrame(by_country['Italy'])
dfi['country'] = 'Italy'
print(dfi)
dffi = pd.concat([df, dfi], ignore_index=True)
print(dffi)
```
...
```{code-cell} ipython3
for i in by_country:
    dfo = pd.DataFrame(by_country[i])
    dfo['country'] = i
    dffi = pd.concat([dffi, dfo], ignore_index=True)
...
```{code-cell} ipython3
dffi['date'] = pd.to_datetime(dffi['date'])
print(dffi)
```
...
#print(clean_df.index.levels)
```

```{code-cell} ipython3
print(clean_df.loc[('France', '2021-01-01')])
```
...
```{code-cell} ipython3
print(clean_df.loc[['France', 'Italy']])
```
...
```{code-cell} ipython3
dfdc = dffr
dfdc = dfdc.drop(columns='recovered')
dfdc.plot();
dfdr = dffr
dfdr = dfdr.drop(columns='confirmed')
dfdr.plot();
```
...
df2 = clean_df.loc[['France', 'Italy', 'Germany']]
df3 = pd.pivot_table(
    df2,
...
df3.plot();
df4 = df3.drop(columns='confirmed')
df4.plot();
```
...
def extract(pays, mesures, debut, fin):
    df7 = clean_df.loc[pd.IndexSlice[pays, debut:fin], :]
    df8 = pd.pivot_table(
        df7,
        values=mesures,
        index='date',
...
```{code-cell} ipython3
extract(['Afghanistan', 'Zimbabwe'], ['deaths'], '2020-01-01', '2022-12-31');
```

en utilisant cette fonction, plottez sur un même graphique les données de deux pays

```{code-cell} ipython3
extract(['Spain', 'France', 'Germany', 'Italy', 'Poland'], ['confirmed'], '2020-01-01', '2022-12-31');
```

```{code-cell} ipython3
extract(['Spain', 'France', 'Germany', 'Italy', 'Poland'], ['deaths'], '2020-01-01', '2022-12-31');
```

```{code-cell} ipython3
extract(['Japan', 'France', 'China'], ['recovered'], '2020-01-01', '2022-12-31');
```

```{code-cell} ipython3
extract(['Japan', 'France', 'China'], ['confirmed'], '2020-01-01', '2022-12-31');
```

## proposez des analyses personnelles sur ces données

```{code-cell} ipython3
# La dernière figure nous permet de voir à quel point les données chinoises étaient biaisées.
# C'est bien de Chine qu'est parti le virus dès le dernier trimestre de 2019,
# et avec des foyers de contaminations très denses, ce qui n'est pas reflété sur le graphique.
```

```{code-cell} ipython3
# On voit aussi la difficulté à gérer la crise sanitaire en France, avec une très forte augmentation des cas en janvier 2022
# soit deux ans après le début de la pandémie. En Europe (sur les pays étudiés), c'est la France qui semble 
# avoir eu le plus de mal à contenir l'épidémie. Néanmoins, le ratio de mort par cas confirmé y était tout de même meilleur qu'en Italie.
```

```{code-cell} ipython3
# On constate un manque de data sur les "recovered" qui intervient vers Aout 2021 et touchant tous les pays.
```

```{code-cell} ipython3
# Il aurait pu être utile de rajouter des colonnes de ratio dans la dataframe afin de comparer
# les différents pays relativement aux cas déclarés. 
# Si deux pays comptent 10'000 décès du covid, on pourrait penser que leur gestion de la crise est
# similaire. Mais si c'est 10'000 décès pour 10'000 cas confirmés ou 10'000 décès pour 1'000'000 de cas
# confirmés, on se doute que la gestion n'est pas la même.
# C'est donc ce que j'ai fait :
```

```{code-cell} ipython3
clean_df2 = clean_df.copy()
clean_df2['ratio d/c'] = clean_df2['deaths'] / clean_df2['confirmed'].replace(0, pd.NA)
def extract2(pays, mesures, debut, fin):
    df9 = clean_df2.loc[pd.IndexSlice[pays, debut:fin], :]
    df10 = pd.pivot_table(
        df9,
        values=mesures,
        index='date',
...
```{code-cell} ipython3
extract2(['Spain', 'France', 'Germany', 'Italy', 'Poland'], ['ratio d/c'], '2020-01-01', '2021-01-01');
```

```{code-cell} ipython3
# Ici, le domaine d'étude est restreint à l'arrivée de la pandémie en Europe, i.e. l'année 2020.
# On voit que malgré des pics (du bruit dans les data?) la France et l'Italie ont un ratio de
# décès par cas confirmés plus haut (0.15) que les autre pays européens, suivis de près par l'Espagne.
# En revanche, la Pologne (pourtant plus pauvre) et l'Allemagne présentent un ratio plus faible (0.05).
```

```{code-cell} ipython3
extract2(['Japan', 'China', 'France'], ['ratio d/c'], '2020-01-01', '2022-06-01');
```

```{code-cell} ipython3
# En comparant ce même ratio entre la France, le Japon et la Chine, on remarque une fois de plus à quel point les données chinoises ont été lissées, tandis que l'on assiste à une quasi corrélation
# entre les vagues "d'efficacité" au Japon et en France.
# En plus du ratio d/c, on aurait pu aussi faire une colonne "dérivée" pour comparer les taux de variation des différentes mesures entre les différents pays, ce qui aurait peut-être permis de mettre
# en évidence les confinements et autres mesures prises par les gouvernements.
```

```{code-cell} ipython3
# L'utilité de ces données me semble assez discutable sur plusieurs points :
#   - Le manque de données (cf la disparition des "recovered" en 2022")
#   - La manque de transparence(?) (cf la Chine)
#   - Le manque de fiabilité (cf les pics français qui ressemblent à du bruit dans les données.
```
